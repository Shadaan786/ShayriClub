...
const categoryStyle = (type) => {
  ...
  const key = (type || "").toLowerCase();
  return map[key] || { label: type || "Kalam" };
};
...
export const KalamCard = ({ kalams = [], onSelect }) => {
  ...
  if (!kalams.length) {
    return (
      <div
        ...
        className="py-10 text-center text-white/20 text-[13px]"
      >
        No kalams yet.
      </div>
    );
  ...
  return (
    <div style={{ fontFamily: "'Outfit', sans-serif" }} className="w-full">
        ...
        {kalams.map((item, i) => {
          ...
          return (
            <button
              ...
              onMouseEnter={() => setHovered(i)}
              onMouseLeave={() => setHovered(null)}
              className="group relative flex items-center gap-6 p-6 rounded-2xl bg-gradient-to-r from-amber-400 to-orange-400 text-black text-left transition-all duration-300 ease-out cursor-pointer"
              style={{
                transform: isHov ? "scale(1.02)" : "scale(1)",
                boxShadow: isHov ? "0 0 30px rgba(249,189,34,0.2)" : "none",
              }}
            >
              {/* index */}
              <span className="text-2xl italic opacity-40 flex-shrink-0 font-light">
                {String(i + 1).padStart(2, "0")}
              </span>

              {/* title + line */}
              <div className="flex-1 min-w-0">
                <h3 className="text-xl font-extrabold truncate">{k.name || "Untitled"}</h3>
                <p className="text-sm italic opacity-80 truncate">
                  {k.content ? `"${truncate(k.content, 70)}"` : ""}
                </p>
              </div>

              {/* category + date */}
              <div className="hidden md:flex flex-col items-end gap-1 px-4 border-l border-black/10 flex-shrink-0">
                <span className="px-2 py-0.5 bg-black/10 rounded-md text-[10px] font-bold uppercase tracking-wider">
                  {cat.label}
                </span>
                <span className="text-[10px] font-medium opacity-60">{formatDate(k.createdAt)}</span>
              </div>

              {/* play button */}
              <div
                className="w-12 h-12 rounded-full flex items-center justify-center shadow-lg transition-transform flex-shrink-0"
                style={{
                  background: "linear-gradient(to bottom right, #000000, #1a0828, #000000)",
                  transform: isHov ? "scale(1.1)" : "scale(1)",
                }}
              >
                <Play className="w-4 h-4 text-amber-400 fill-amber-400" strokeWidth={0} />
              </div>
            </button>
          );
        })}
      ...
      {/* footer count */}
      <div className="mt-4 px-2 flex items-center justify-between border-t border-white/[0.06] pt-3">
