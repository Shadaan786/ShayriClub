...
useEffect(() => {
  ...
  const handleNotificationSeen=(notificationId)=>{
    ...
    useEffect(() => {
    const track = document.getElementById('featureScroll');
    if (!track) return;
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          ...
          entry.target.style.setProperty('animation-play-state', state);
    ...
    <div className="md:hidden bg-black/95 backdrop-blur-lg border-t border-white/10">
      <div className="px-4 py-4 space-y-1">
        ...
        <div className="pt-4 mt-2 space-y-2 border-t border-white/10">
          ...
          {isLoggedIn && (
            <div className="px-2">
              <Dropdown title={"Profile"} items={{menuItems,profilePic}} />
            </div>
          )}
        </div>
      </div>
...
<section className="relative w-full" style={{ marginTop: '80px' }}>
  ...
  <div
    ...
    style={{ scrollbarWidth: 'none', cursor: 'grab', willChange: 'scroll-position' }}
    onMouseDown={(e) => {
      const el = document.getElementById('featureScroll');
      el._isDragging = true;
      el._startX = e.pageX;
      ...
      // Disable snap while dragging so the browser doesn't fight our
      // manual scrollLeft writes — this is the main source of jank.
    ...
    onMouseMove={(e) => {
      ...
      e.preventDefault();
      const x = e.pageX;
      // Coalesce rapid mousemove events into a single write per frame
      // instead of writing scrollLeft on every mousemove (can fire
      // 100+ times/sec, which is the biggest cause of stutter).
      if (el._raf) return;
      el._raf = requestAnimationFrame(() => {
        el.scrollLeft = el._scrollLeft - (x - el._startX);
        el._raf = null;
      });
    }}
    onMouseUp={() => {
...
{/* ── Slide 1: Audio Kalam ── */}
<div
  className="relative flex-shrink-0 w-full h-screen snap-start flex items-center justify-center overflow-hidden min-h-[560px]"
...
<div className="flex-1 flex items-center justify-center h-full py-4 relative">
  ...
  <div className="relative z-10" style={{ transform: 'rotate(2deg)' }}>
    ...
    <div className="absolute -top-4 -left-8 z-20 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold text-white"
      style={{ background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(6px)', border: '1px solid rgba(255,255,255,0.2)', whiteSpace: 'nowrap' }}>
      🎨 120+ Themes
    </div>
    {/* Top-right */}
    ...
    <div className="absolute -bottom-4 -left-8 z-20 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold text-white"
      style={{ background: 'rgba(168,85,247,0.25)', backdropFilter: 'blur(6px)', border: '1px solid rgba(168,85,247,0.4)', whiteSpace: 'nowrap' }}>
      ✨ Live Preview
    </div>
    {/* Bottom-right */}
...
>
  <style>{`
    ...
    @media (prefers-reduced-motion: reduce) {
      .kotw-floating-folio, .kotw-wave-reed, .kotw-animate-spotlight-beam,
      .kotw-animate-aura-halo, .kotw-mote-1, .kotw-mote-2, .kotw-mote-3,
      .kotw-mote-4, .kotw-mote-5 { animation: none !important; }
    }
  `}</style>
            ...
            <div className="rounded-lg bg-[#12151d]/90 border border-[#222734] p-5 sm:p-7 shadow-2xl backdrop-blur-md archival-animate-showcase-float">
              ...
              <div className="grid grid-cols-3 gap-3.5 sm:gap-4 mb-5">
                ...
                <div className="archival-vinyl-sleeve archival-animate-float-2 group rounded-sm overflow-hidden aspect-square bg-[#161a22] cursor-pointer border border-white/10 relative">
                  <img alt="Motivation and Fire" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" src="https://picsum.photos/seed/aatish-e-junoon/600/600" loading="lazy" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/20 to-black/30 p-2.5 flex flex-col justify-between">
                    <div className="flex items-center justify-between">
                ...
                <div className="archival-vinyl-sleeve archival-animate-float-3 group rounded-sm overflow-hidden aspect-square bg-[#161a22] cursor-pointer border border-white/10 relative">
                  <img alt="Aurora Echoes" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" src="https://picsum.photos/seed/sadaa-e-kohsaar/600/600" loading="lazy" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/20 to-black/30 p-2.5 flex flex-col justify-between">
                    <div className="flex items-center justify-between">
